...
get '/graphdata/:screen_name/by/:interval' => sub {
    ...
    my $end_of_frame   = DateTime->from_epoch( epoch => $creation->epoch )->add(%addition);
    my @data_points;
    my $sum             = 0;
    my @cumulatives     = ();
    ...
    while ( $start_of_frame < $now ) {
        my $tweet_count = $twitter_account->tweets->search(
            { tweeted_at => { '>=', $start_of_frame->ymd } } )
          ->search( { tweeted_at => { '<', $end_of_frame->ymd } } )->count;
        $sum += $tweet_count;
...
get '/graph/:screen_name/by/:interval' => sub {
    ...
    my $profile_image = get_twitter_account($screen_name)->profile_image_url
      || '/images/squirrel_icon64.gif';
    my $graphdataurl = proxy->uri_for("/graphdata/$screen_name/by/$interval");
